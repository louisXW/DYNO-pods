...
class SyncStrategyNoConstraintsDYNOPODS(object):
    ...
    def __init__(self, obj_func, worker_id, data, response_surface, maxeval, nsamples,
                 exp_design=None, sampling_method=None, extra=None, extra_vals=None, ini_xs=None, ini_vals=None):
        ...
        self.sigma = 0
        self.xbest = None
        self.fbest = [np.inf]*self.data.no_sub_objs
        self.fbest_old = None
        self.ini_xs = ini_xs
    ...
    def log_completion(self, obj, x, simid, iterid):
        ...
        print("{} {} @ {}".format("evaluated simulation %s:"%(simid), obj, xstr))

        logger.info("{} {} @ {}".format("True", obj, xstr))
    ...
    def adjust_step(self):
        ...
        for i in range(self.data.no_sub_objs):
            if self.fbest[i] < self.fbest_old[i] - 1e-3 * math.fabs(self.fbest_old[i]):
                no_improved_objs +=1
        if no_improved_objs > 0:
    ...
    def on_complete(self, objfuns, parameters):
        ...
        for indx, item in enumerate(objfuns):
            ...
            self.parameters.append(params[indx])
        ...
        if self.data.no_sub_objs > 1:
            ...
            for i in range(self.data.no_sub_objs):
                ...
                obj = obj + (sub_objs[:, i] - min(sub_objs[:, i])) / float(
                    np.median(sub_objs[:, i]) - min(sub_objs[:, i]))  # Modified Jul 25
