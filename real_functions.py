...
class delft3d_1objs:
    ...
    def __init__(self, dim=4):
        ...
        self.no_sub_objs = 1
        self.home_dir = ''
        self.file_result = 'pysot_result.txt'
        self.project_name = 'f34'
        self.file_save = True
    ...
    def objfunction(self, x, simid, iterid):
        ...
        parms = x.tolist()
        fp = open(self.home_dir + "/result/" + self.file_result, "a")
    ...
    def delft3d_flow(self, x, simid, iterid):
        ...
        try:
           ...
           subprocess.call(cmd, cwd=workingdir)
            ...
            if self.file_save:
                self.save_result_file(simid_lloc, iterid_lloc)
    ...
    def modify_coefficient(self, x, sim_id, iter_id):
        ...
        fp = open(workingdir + "/%s_base.mdf" %self.project_name, "rb")
        ...
        fp.close()
        fp = open(workingdir + "/%s.mdf" %self.project_name, "wb")
    ...
    def save_result_file(self, sim_id, iter_id):
        ...
        command = "cp %s/trih-%s.dat %s/result/history_data/%s_%strih-%s.dat" % (
            workingdir, self.project_name, self.home_dir, simiter_name, sim_id, self.project_name)
        subprocess.call(command, shell=True, cwd=workingdir)

        command = "cp %s/trih-%s.def %s/result/history_data/%s_%strih-%s.def" % (
            workingdir, self.project_name, self.home_dir, simiter_name, sim_id, self.project_name)
        subprocess.call(command, shell=True, cwd=workingdir)
    ...
    def delft3d_flow(self, x, simid, iterid):
        ...
        try:
           ...
           subprocess.Popen(cmd, cwd=workingdir).wait()
            ...
            observation_curw_layer1 = postutil.read_simulation_data(observation_curw, 1, 1)
            sim_curu_layer1 = postutil.read_simulation_data(sim_curu, 1, 1)
            sim_curv_layer1 = postutil.read_simulation_data(sim_curv, 1, 1)
            sim_curw_layer1 = postutil.read_simulation_data(sim_curw, 1, 1)
            ...
            observation_curu_layer2 = postutil.read_simulation_data(observation_curu, 2, 1)

            observation_curv_layer2 = postutil.read_simulation_data(observation_curv, 2, 1)

            observation_curw_layer2 = postutil.read_simulation_data(observation_curw, 2, 1)
